...
"""Validate and summarize MirrorArc agent-readiness benchmark task packs."""
...
MODE_ORDER = ("raw_source_folder", "plain_markitdown_dump", "mirrorarc_markdown")
REQUIRED_MODES = set(MODE_ORDER)
LEGACY_REQUIRED_MODES = {"raw_source_folder", LEGACY_DOCUMENT_CHAT_MODE, "mirrorarc_markdown"}
...
def validate_task_pack(path: Path, *, require_generated: bool = False) -> tuple[dict, list[str], list[str]]:
    ...
    if not isinstance(modes, list) or len(mode_names) != len(mode_set):
        ...
        if LEGACY_PRODUCT_MODE in mode_set:
            warnings.append("comparison_modes uses legacy vaultwright_markdown; prefer mirrorarc_markdown")
    elif mode_set != REQUIRED_MODES:
        errors.append("comparison_modes must include raw_source_folder, plain_markitdown_dump, and mirrorarc_markdown")
...
def task_scaffold_data(source_pairs: list[dict[str, str]], curated_paths: list[str]) -> dict:
    ...
    return {
        ...
        "corpus": ROOT.name,
        "description": (
            "Private benchmark task scaffold generated from MirrorArc manifest metadata. "
...
def result_scaffold_text(task_data: dict) -> tuple[str, int]:
    ...
    lines = [
        "# MirrorArc agent-readiness result scaffold.",
    ...
    for task in tasks:
        ...
        for mode in comparison_modes_from_task_pack(task_data):
            ...
            if mode in {"mirrorarc_markdown", LEGACY_PRODUCT_MODE}:
