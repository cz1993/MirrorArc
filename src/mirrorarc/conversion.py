...
def load_source_records(root: Path) -> tuple[list[dict], list[str], list[str]]:
    ...
    if not path.exists():
        return [], [f"{SOURCE_MANIFEST.as_posix()}: missing; run `mirrorarc sync` first."], []
    try:
    ...
    sections = [
        {
            "title": "Preflight",
            "items": [
                "Run `mirrorarc status` and `mirrorarc recovery` before sign-off.",
...
def validate_quality_results(
    ...
    *,
    ...
    if not (root / SOURCE_MANIFEST).exists():
        return summary, [], [f"{SOURCE_MANIFEST.as_posix()}: missing; run `mirrorarc sync` first."]
...
def print_human(
    ...
    print(f"mirrorarc conversion: {root}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Print a read-only MirrorArc conversion spot-check report.")
