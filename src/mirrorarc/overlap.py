...
def print_text(report: dict[str, Any], errors: list[str]) -> None:
    ...
    print("  - If real duplicates are missing, lower thresholds in the copied pilot vault and rerun `mirrorarc overlap`.")


def print_worksheet(report: dict[str, Any], errors: list[str]) -> None:
    summary = report["summary"]
    config = report["config"]
    print("# MirrorArc Overlap Calibration Worksheet")
    print("")
    print("Generated by `mirrorarc overlap --worksheet`. Read-only; no files were changed.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a MirrorArc overlap calibration report.")
