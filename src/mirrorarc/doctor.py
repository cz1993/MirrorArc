...
"""Package-owned MirrorArc doctor checks."""
...
from mirrorarc.views import profile_views_plan
...
GITIGNORE_REQUIRED_PATTERNS = {
    "data/": "data/.mirrorarc-doctor-check",
    "secrets/": "secrets/.mirrorarc-doctor-check",
    "private/": "private/.mirrorarc-doctor-check",
    ".env": ".env",
    "*.pem": "mirrorarc-doctor-check.pem",
...
def profile_view_preflight(root: Path) -> tuple[list[str], list[str]]:
    ...
    for view in plan["views"]:
        ...
        if state == "current":
            ...
            warnings.append(f"profile view: {path} {state}; run `mirrorarc profile views --write`.")
        else:
...
def review_preflight(root: Path) -> tuple[list[str], list[str]]:
    ...
    if stale_or_missing:
        warnings.append(
            ...
            "run `mirrorarc review`."
        )
    if non_approved:
        warnings.append(
            f"review ledger: {non_approved} reviewed artifact(s) are not approved; run `mirrorarc review`."
        )
    return info, warnings
...
def print_report(report: dict[str, object]) -> None:
    print(f"mirrorarc doctor: {report['root']}")
