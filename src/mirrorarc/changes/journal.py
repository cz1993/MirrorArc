...
from mirrorarc.changes.events import (
    COUNTED_STATUSES,
    EVENT_STATUSES,
    JournalEventError,
...
class JournalSchemaError(JournalError):
    """Raised when local journal schema metadata needs a newer MirrorArc."""
...
def _migrate_schema(conn: sqlite3.Connection, meta: dict[str, str]) -> None:
    ...
    raise JournalSchemaError(
        f"journal schema_version {found} is newer than this MirrorArc supports; "
        "run a newer MirrorArc before replaying local journal work"
    )
