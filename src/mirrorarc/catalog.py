...
"""Generate a source-path-only MirrorArc documentation catalog."""
...
def render_html(report: dict[str, Any], warnings: list[str], errors: list[str], max_items: int = 500) -> str:
    ...
    lines = [
        ...
        '<p class="muted">Generated by <code>mirrorarc catalog --html</code>. This catalog lists paths and manifest metadata only; it does not copy source document text.</p>',
        "</header>",
    ...
    if source_items:
        ...
        lines.append('<p class="empty">No source manifest records yet. Run <code>mirrorarc sync</code> after reviewing <code>mirrorarc plan</code>.</p>')
    lines.append("</section>")
    ...
    lines.extend(
        [
            ...
            "<ul>",
            "<li>Run <code>mirrorarc sandbox --source-root &lt;original-source-root&gt;</code> before using a copied pilot vault.</li>",
            "<li>Run <code>mirrorarc conversion --guide</code> after sync to spot-check high-risk formats.</li>",
            "<li>Use <code>mirrorarc migration</code> before moving legacy folders into canonical domains.</li>",
