...
"""Generate MirrorArc documentation catalogs and the local HTML explorer."""
...
HTML_CONTENT_PER_DOCUMENT_LIMIT = 400_000
HTML_CONTENT_TOTAL_LIMIT = 8_000_000
...
def active_profile_summary(root: Path) -> dict[str, str]:
    path = root / PROFILE
    if not path.exists():
        return {"id": "legacy-workspace", "name": "Legacy workspace", "description": ""}
    try:
        profile = load_profile(path)
    except ProfileValidationError:
        return {"id": "invalid-profile", "name": "Profile needs review", "description": ""}
    return {
        "id": profile.id,
        "name": profile.name,
...
def markdown_catalog_item(
    ...
    return {
        ...
        "domain": str(metadata.get("domain") or domain_for_path(rel, aliases)),
    }
...
def workspace_inventory(root: Path, aliases: dict[str, str], content_roots: set[str]) -> dict[str, Any]:
    ...
    machine_owned_items: list[dict[str, str]] = []
    generated_mirrors = 0
    curated_markdown = 0
    ...
    for path in sorted(root.rglob("*")):
        ...
        if suffix == ".md":
            ...
            item = markdown_catalog_item(path, root, aliases, metadata)
            if str(metadata.get("type", "") or "") in machine_owned_note_types:
                machine_owned_markdown_files.append(rel.as_posix())
                machine_owned_items.append(item)
            else:
        ...
        items.append(
            {
                ...
                "normalized_content_sha256": str(record.get("normalized_content_sha256", "") or ""),
                "converter": str(record.get("converter", "") or record.get("converter_name", "") or ""),
                "converter_version": str(record.get("converter_version", "") or ""),
                "last_successful_sync": str(record.get("last_successful_sync", "") or ""),
            }
        )
        ...
        items.append(
            {
                ...
                "errors": len(record.get("errors", [])) if isinstance(record.get("errors"), list) else 0,
                "domain": domain_for_path(note, aliases),
                "commit": str(record.get("commit") or record.get("resolved_commit") or record.get("head") or ""),
                "last_successful_sync": str(record.get("last_successful_sync", "") or ""),
            }
        )
...
def build_report(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
        ...
        domain_rows.append(
            {
                "domain": "unclassified",
                "folder": "",
                "purpose": "Source files outside configured MirrorArc domains.",
    ...
    report = {
        "profile": active_profile_summary(root),
        "summary": {
...
def html_document_content(root: Path, report: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Collect bounded Markdown bodies for an explicitly content-enabled local explorer."""
    candidates = {"INDEX.md"}
    for key in ("curated_items", "machine_owned_items"):
        for item in report.get(key, []):
            if isinstance(item, dict):
                candidates.add(str(item.get("path") or ""))
    for item in report.get("source_items", []):
        if isinstance(item, dict):
            candidates.add(str(item.get("mirror") or ""))
    ...
    for value in sorted(candidates):
        ...
        try:
            ...
            continue
        remaining = HTML_CONTENT_TOTAL_LIMIT - total
        if remaining <= 0:
            break
        limit = min(HTML_CONTENT_PER_DOCUMENT_LIMIT, remaining)
        rendered = text[:limit]
        content[rel] = {
            "text": rendered,
...
def render_markdown(report: dict[str, Any], warnings: list[str], errors: list[str], max_items: int = 500) -> str:
    ...
    lines = [
        ...
        "",
        "Generated by `mirrorarc catalog`. This catalog lists paths and manifest metadata only; it does not copy source document text.",
        "",
        "## Inventory Summary",
        "",
        *table(
    ...
    if source_items:
        ...
        lines.append("- No source manifest records yet. Run `mirrorarc sync` after reviewing `mirrorarc plan`.")
    lines.append("")
    ...
    lines.extend(
        [
            ...
            *[f"- {md_escape(item)}" for item in report["prompt_safety"]],
            ...
            "## Operator Next Steps",
            "",
            "- Run `mirrorarc sandbox --source-root <original-source-root>` before using a copied pilot vault.",
            "- Run `mirrorarc conversion --guide` after sync to spot-check high-risk formats.",
            "- Use `mirrorarc migration` before moving legacy folders into canonical domains.",
...
def render_html(report: dict[str, Any], warnings: list[str], errors: list[str], max_items: int = 500) -> str:
    """Render the self-contained MirrorArc Catalog Explorer."""
    return render_catalog_explorer(report, warnings, errors, max_items=max_items)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a MirrorArc documentation catalog.")
...
def main(argv: list[str] | None = None, *, root: Path | None = None) -> int:
    ...
    if args.include_content and not args.html:
        parser.error("--include-content requires --html")
