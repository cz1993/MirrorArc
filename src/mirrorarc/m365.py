...
def build_report(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    ...
    if not source_records:
        readiness.append({"level": "review", "message": "Run `mirrorarc sync` before handoff so source mirrors and manifests exist."})
    if not catalogs["markdown"]["present"]:
        readiness.append({"level": "review", "message": "Run `mirrorarc catalog` before handoff so reviewers have a Markdown gateway."})
    if not catalogs["html"]["present"]:
        readiness.append({"level": "review", "message": "Run `mirrorarc catalog --html` before handoff so reviewers have a browser gateway."})
    ...
    if not readiness:
        readiness.append({"level": "ok", "message": "Core MirrorArc handoff artifacts are present; review Microsoft 365 permissions and tenant settings next."})
    ...
    report = {
        ...
        "microsoft_365_notes": [
            ...
            "Keep original records in approved SharePoint/OneDrive locations; MirrorArc mirrors are derived working copies.",
