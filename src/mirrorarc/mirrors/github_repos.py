...
`mirrorarc migrate annotations --write` before sync refreshes the mirror. Only the auto region
...
def api_get(path, token):
    req = urllib.request.Request(
        ...
        headers={"Accept": "application/vnd.github+json", "User-Agent": "mirrorarc-sync"},
    )
    if token:
