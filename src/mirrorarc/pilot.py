...
from mirrorarc.runtime_profile import configured_office_mirror_root
...
def conversion_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    try:
        ...
        messages = [f"conversion report has {len(warnings)} warnings; run `mirrorarc conversion`"] if warnings else []
        failures = [f"conversion report has {len(errors)} errors; run `mirrorarc conversion`"] if errors else []
...
def conversion_quality_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    ...
    try:
        ...
        summary["warnings"] = len(warnings)
        summary["errors"] = len(errors)
        quality_command = f"mirrorarc conversion --results {CONVERSION_QUALITY_RESULTS.as_posix()}"
        messages = [
...
def recovery_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    try:
        ...
        summary["warnings"] = len(warnings)
        summary["errors"] = len(errors)
        messages = [f"recovery report has {len(warnings)} warnings; run `mirrorarc recovery`"] if warnings else []
        failures = [f"recovery report has {len(errors)} errors; run `mirrorarc recovery`"] if errors else []
        return {"available": True, "summary": summary}, messages, failures
    except Exception as exc:
...
def overlap_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    try:
        ...
        summary["warnings"] = len(warnings)
        messages = [f"overlap report has {len(warnings)} warnings; run `mirrorarc overlap`"] if warnings else []
...
def benchmark_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    try:
        ...
        messages = [f"benchmark report has {len(warnings)} warnings; run `mirrorarc benchmark`"] if warnings else []
        failures = [f"benchmark report has {len(errors)} errors; run `mirrorarc benchmark`"] if errors else []
        return {"available": True, "summary": summary}, messages, failures
    except Exception as exc:
...
def review_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    try:
        ...
        messages = [f"review ledger has {len(warnings)} warnings; run `mirrorarc review`"] if warnings else []
...
def print_worksheet_summary(report: dict[str, Any], warnings: list[str], errors: list[str]) -> None:
    ...
    print("# MirrorArc Pilot Evidence Summary")
    print()
    print(
        "Generated from aggregate MirrorArc manifests and reports only. It intentionally omits "
    ...
    print("- MirrorArc time to answer fixed questions:")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Print a read-only MirrorArc pilot evidence report.")
