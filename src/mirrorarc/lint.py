...
"""
lint_vault.py — health check for a MirrorArc knowledge base.
...
def main(root: Path | None = None) -> int:
    ...
    def is_repo_mirror_path(rel: Path) -> bool:
        ...
        for notes_dir in runtime_repo_notes_dirs(ROOT):
            try:
                repo_root = repo_note_path(notes_dir, "__mirrorarc_repo_probe__.md").parent
    ...
    def mirror_annotation_migration_issue(rel: Path, note_type: str, fm: dict, body: str) -> str | None:
        ...
        if matching_annotation_sidecar(rel, note_type, fm, preserved):
            return None
        return "above-sentinel annotations need sidecar migration; run mirrorarc migrate annotations --write"
    ...
    def source_mirror_freshness_issue(rel: Path, fm: dict) -> str | None:
        ...
        if state not in CURRENT_SOURCE_STATES:
            return f"source-manifest lifecycle_state={state}; run mirrorarc sync/status before relying on mirror"
        try:
        ...
        if expected_sha and current_sha != expected_sha:
            return "source hash changed; run mirrorarc sync before relying on mirror"
        return None
    ...
    def repo_mirror_freshness_issue(fm: dict) -> str | None:
        ...
        if manifest_repos and mirror_repo not in manifest_repos:
            return "repo frontmatter repo differs from repo manifest; run mirrorarc sync before relying on mirror"
        state = str(record.get("lifecycle_state", "clean") or "clean")
        if state not in CURRENT_REPO_STATES:
            return f"repo-manifest lifecycle_state={state}; run mirrorarc sync/status before relying on mirror"
        ...
        if manifest_commit and mirror_commit and manifest_commit != mirror_commit:
            return "repo manifest last_commit differs from mirror frontmatter; run mirrorarc sync before relying on mirror"
        ...
        if expected_commit and current_commit != expected_commit:
            return "local repo tree changed; run mirrorarc sync before relying on mirror"
        return None
    ...
    for expected in CONFIGURED_REPOS:
        ...
        if actual_repo_id != expected_repo_id:
            repo_mirror_gap.append(str(note_path.relative_to(ROOT)) + "  (configured repo mirror repo_id mismatch; run mirrorarc sync)")
