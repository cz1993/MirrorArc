...
def print_text(report: dict[str, Any], errors: list[str]) -> None:
    ...
    print("  - If real duplicates are missing, lower thresholds in the copied pilot vault and rerun `noeticweave overlap`.")


def print_worksheet(report: dict[str, Any], errors: list[str]) -> None:
    summary = report["summary"]
    config = report["config"]
    print("# NoeticWeave Overlap Calibration Worksheet")
    print("")
    print("Generated by `noeticweave overlap --worksheet`. Read-only; no files were changed.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a NoeticWeave overlap calibration report.")
