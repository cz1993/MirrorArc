...
"""Generate a source-path-only NoeticWeave documentation catalog."""
        ...
        domain_rows.append(
            {
                "domain": "unclassified",
                "folder": "",
                "purpose": "Source files outside configured NoeticWeave domains.",
...
def render_markdown(report: dict[str, Any], warnings: list[str], errors: list[str], max_items: int = 500) -> str:
    ...
    lines = [
        ...
        "",
        "Generated by `noeticweave catalog`. This catalog lists paths and manifest metadata only; it does not copy source document text.",
        "",
        "## Inventory Summary",
        "",
        *table(
    ...
    if source_items:
        ...
        lines.append("- No source manifest records yet. Run `noeticweave sync` after reviewing `noeticweave plan`.")
    lines.append("")
    ...
    lines.extend(
        [
            ...
            *[f"- {md_escape(item)}" for item in report["prompt_safety"]],
            ...
            "## Operator Next Steps",
            "",
            "- Run `noeticweave sandbox --source-root <original-source-root>` before using a copied pilot vault.",
            "- Run `noeticweave conversion --guide` after sync to spot-check high-risk formats.",
            "- Use `noeticweave migration` before moving legacy folders into canonical domains.",
...
def render_html(report: dict[str, Any], warnings: list[str], errors: list[str], max_items: int = 500) -> str:
    ...
    lines = [
        ...
        '<p class="muted">Generated by <code>noeticweave catalog --html</code>. This catalog lists paths and manifest metadata only; it does not copy source document text.</p>',
        "</header>",
    ...
    if source_items:
        ...
        lines.append('<p class="empty">No source manifest records yet. Run <code>noeticweave sync</code> after reviewing <code>noeticweave plan</code>.</p>')
    lines.append("</section>")
    ...
    lines.extend(
        [
            ...
            "<ul>",
            "<li>Run <code>noeticweave sandbox --source-root &lt;original-source-root&gt;</code> before using a copied pilot vault.</li>",
            "<li>Run <code>noeticweave conversion --guide</code> after sync to spot-check high-risk formats.</li>",
            "<li>Use <code>noeticweave migration</code> before moving legacy folders into canonical domains.</li>",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a NoeticWeave documentation catalog.")
