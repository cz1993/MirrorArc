...
from noeticweave.changes import fingerprint, journal
from noeticweave.changes.events import JournalEventError, normalize_vault_relative_path, validate_event_kind
from noeticweave.runtime_profile import configured_office_mirror_root, rel_is_under, repo_notes_dirs

LOCAL_STATE_DIRS = {Path(".noeticweave"), Path(".vaultwright")}
OPERATIONAL_DIRS = {
    ".git",
    ".githooks",
    ".github",
...
def ignored_reason(root: Path, value: str | Path | None) -> str | None:
    ...
    for state_dir in LOCAL_STATE_DIRS:
        if rel == state_dir or rel_is_under(rel, state_dir):
            return "local-state"
