...
from noeticweave.changes.events import (
    COUNTED_STATUSES,
    EVENT_STATUSES,
    JournalEventError,
...
STATE_DIR = Path(".noeticweave")
STATE_DB = STATE_DIR / "state.sqlite"
LEGACY_STATE_DIR = Path(".vaultwright")
LEGACY_STATE_DB = LEGACY_STATE_DIR / "state.sqlite"
SCHEMA_VERSION = 1
...
class JournalSchemaError(JournalError):
    """Raised when local journal schema metadata needs a newer NoeticWeave."""
...
def state_db_path(root: Path) -> Path:
    current = root / STATE_DB
    legacy = root / LEGACY_STATE_DB
    if not current.exists() and legacy.exists():
        return legacy
    return current


def display_state_db_path(root: Path, path: Path) -> str:
    try:
...
def _migrate_schema(conn: sqlite3.Connection, meta: dict[str, str]) -> None:
    ...
    raise JournalSchemaError(
        f"journal schema_version {found} is newer than this NoeticWeave supports; "
        "run a newer NoeticWeave before replaying local journal work"
    )
...
def _schema_error_status(root: Path, path: Path, error: JournalError) -> dict[str, Any]:
    ...
    payload.update(
        {
            "state_path": display_state_db_path(root, path),
