...
"""Installable NoeticWeave console entry point.
...
from noeticweave import benchmark as benchmark_module
from noeticweave import catalog as catalog_module
from noeticweave import conversion as conversion_module
from noeticweave import doctor as doctor_module
from noeticweave import lint as lint_module
from noeticweave import m365 as m365_module
from noeticweave import migration as migration_module
from noeticweave import overlap as overlap_module
from noeticweave import pilot as pilot_module
from noeticweave import recovery as recovery_module
from noeticweave import review_ledger as review_ledger_module
from noeticweave import sandbox as sandbox_module
from noeticweave.annotation_migration import (
    annotation_migration_plan,
    public_plan,
    write_annotation_sidecars,
)
from noeticweave.changes import changed_sync as changed_sync_module
from noeticweave.changes import feed as feed_module
from noeticweave.changes import journal as journal_module
from noeticweave.changes import native_watch as native_watch_module
from noeticweave.changes import reconcile as reconcile_module
from noeticweave.changes import replay as replay_module
from noeticweave.changes import watch as watch_module
from noeticweave.mirrors import github_repos as repo_sync_module
from noeticweave.mirrors import office as office_sync_module
from noeticweave.profile_migration import profile_migration_plan, write_profile_migration
from noeticweave.profile_scaffold import BUSINESS_OPERATIONS_PROFILE_ID, scaffold_profile_vault
from noeticweave.profiles import ProfileContract, ProfileValidationError, load_profile
from noeticweave.views import profile_views_plan, write_profile_views
...
def template_source() -> Path | None:
    env_root = os.environ.get("NOETICWEAVE_REPO") or os.environ.get("VAULTWRIGHT_REPO")
    candidates = []
    if env_root:
...
def command_init(args: argparse.Namespace) -> int:
    try:
        ...
        print(f"noeticweave init: {exc}", file=sys.stderr)
        return 1
    ...
    print(f"NoeticWeave vault created at: {target}")
    print(f"Profile: {profile.id} {profile.profile_version}")
    target_arg = shlex.quote(str(target))
    print("Next (with an installed NoeticWeave CLI):")
    print(f"  noeticweave --root {target_arg} doctor")
    print(f"  noeticweave --root {target_arg} plan")
    print(f"  noeticweave --root {target_arg} sync --json")
    print(f"  noeticweave --root {target_arg} status --json")
    return 0
...
def command_plan(args: argparse.Namespace) -> int:
    ...
    if config.exists():
        ...
        print("noeticweave plan: no tools/repos.yml found; repo plan skipped")
    return status
...
def command_watch(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 2
    if not args.json:
        print("noeticweave watch --native: watching " + ", ".join(path.relative_to(root).as_posix() for path in roots))
    observer.start()
    exit_code = 0
    cycles = 0
    try:
        ...
        if not args.json:
            print("noeticweave watch --native: stopped")
    finally:
        observer.stop()
        observer.join()
    return exit_code
...
def command_status(args: argparse.Namespace) -> int:
    ...
    if config.exists():
        ...
        print("noeticweave status: no tools/repos.yml found; repo status skipped")
    return status
...
def command_journal_status(args: argparse.Namespace) -> int:
    ...
    if args.json:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return 0
    print(f"noeticweave journal status: {payload['state_path']}")
    ...
    if not payload.get("schema_supported", True):
        schema_text += " (unsupported by this NoeticWeave)"
...
def command_journal_replay(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        print(f"noeticweave journal replay: processed {payload['processed']} event(s)")
...
def command_reconcile(args: argparse.Namespace) -> int:
    ...
    if args.json:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return 0
    print(f"noeticweave reconcile: queued {payload['events_queued']} event(s)")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="NoeticWeave command-line interface.")
    parser.add_argument("--root", type=Path, default=Path.cwd(), help="Vault root for plan/sync/status/lint/doctor.")
    parser.add_argument("--version", action="version", version=f"noeticweave {__version__}")
    sub = parser.add_subparsers(dest="command", required=True)

    init = sub.add_parser("init", help="Scaffold a new NoeticWeave vault from the template.")
    init.add_argument(
        "--profile",
    ...
    profile = sub.add_parser("profile", help="Inspect and validate NoeticWeave profile contracts.")
    ...
    migrate = sub.add_parser("migrate", help="Run package-owned NoeticWeave migrations.")
