...
"""Validate and summarize NoeticWeave agent-readiness benchmark task packs."""
...
LEGACY_PRODUCT_MODE = "vaultwright_markdown"
MODE_ORDER = ("raw_source_folder", "plain_markitdown_dump", "noeticweave_markdown")
REQUIRED_MODES = set(MODE_ORDER)
LEGACY_REQUIRED_MODES = {"raw_source_folder", LEGACY_DOCUMENT_CHAT_MODE, "noeticweave_markdown"}
LEGACY_BRAND_REQUIRED_MODES = {"raw_source_folder", "plain_markitdown_dump", LEGACY_PRODUCT_MODE}
LEGACY_BRAND_AND_BASELINE_MODES = {"raw_source_folder", LEGACY_DOCUMENT_CHAT_MODE, LEGACY_PRODUCT_MODE}
SUPPORTED_MODE_SETS = (
    REQUIRED_MODES,
    LEGACY_REQUIRED_MODES,
    LEGACY_BRAND_REQUIRED_MODES,
    LEGACY_BRAND_AND_BASELINE_MODES,
)
...
def validate_task_pack(path: Path, *, require_generated: bool = False) -> tuple[dict, list[str], list[str]]:
    ...
    elif mode_set in SUPPORTED_MODE_SETS[1:]:
        if LEGACY_DOCUMENT_CHAT_MODE in mode_set:
            warnings.append("comparison_modes uses legacy document_chat_transcript; prefer plain_markitdown_dump")
        if LEGACY_PRODUCT_MODE in mode_set:
            warnings.append("comparison_modes uses legacy vaultwright_markdown; prefer noeticweave_markdown")
    elif mode_set != REQUIRED_MODES:
        errors.append("comparison_modes must include raw_source_folder, plain_markitdown_dump, and noeticweave_markdown")
...
def task_scaffold_data(source_pairs: list[dict[str, str]], curated_paths: list[str]) -> dict:
    ...
    return {
        ...
        "corpus": ROOT.name,
        "description": (
            "Private benchmark task scaffold generated from NoeticWeave manifest metadata. "
...
def result_scaffold_text(task_data: dict) -> tuple[str, int]:
    ...
    lines = [
        "# NoeticWeave agent-readiness result scaffold.",
    ...
    for task in tasks:
        ...
        for mode in comparison_modes_from_task_pack(task_data):
            ...
            if mode in {"noeticweave_markdown", LEGACY_PRODUCT_MODE}:
...
def comparison_modes_from_task_pack(task_data: dict) -> tuple[str, ...]:
    ...
    mode_set = set(mode_names)
    if len(mode_names) == len(mode_set) and mode_set in SUPPORTED_MODE_SETS:
        return mode_names
    return MODE_ORDER
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a NoeticWeave agent-readiness benchmark task pack.")
    parser.add_argument(
        "--tasks",
        type=Path,
        help=(
