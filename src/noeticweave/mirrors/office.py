...
`noeticweave migrate annotations --write` before sync refreshes the mirror.
...
ANNOTATION_MIGRATION_REQUIRED_WARNING = (
    ...
    "run `noeticweave migrate annotations --write` before syncing."
)
...
AMBIGUOUS_SOURCE_MOVE_ERROR = (
    "Source bytes match multiple missing manifest records; NoeticWeave cannot choose the correct "
...
def print_plan_or_status(
    ...
    *,
    ...
    for plan in plans:
        ...
        items.append(
            {
                "source": as_posix_rel(src.relative_to(root)),
    ...
    payload = {
        ...
        "mode": mode,
        "root": str(root),
        "current_sources": len(files),
        "missing_manifest_sources": missing,
        "action_counts": action_counts,
        "state_counts": state_counts,
        "warnings_count": warning_total,
        "items": items,
    }
    if json_output:
...
def main(argv: list[str] | None = None, default_root: Path | None = None):
    ...
    payload = {
        ...
        "changed": [as_posix_rel(path) for path in changed],
        "items": items,
    }
    if args.json:
        print(json.dumps(payload, indent=2, sort_keys=True))
