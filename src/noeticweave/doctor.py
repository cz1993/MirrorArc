...
"""Package-owned NoeticWeave doctor checks."""
...
from noeticweave.views import profile_views_plan
...
GITIGNORE_REQUIRED_PATTERNS = {
    "data/": "data/.noeticweave-doctor-check",
    "secrets/": "secrets/.noeticweave-doctor-check",
    "private/": "private/.noeticweave-doctor-check",
    ".env": ".env",
    "*.pem": "noeticweave-doctor-check.pem",
...
def profile_view_preflight(root: Path) -> tuple[list[str], list[str]]:
    ...
    for view in plan["views"]:
        ...
        if state == "current":
            ...
            warnings.append(f"profile view: {path} {state}; run `noeticweave profile views --write`.")
        else:
...
def review_preflight(root: Path) -> tuple[list[str], list[str]]:
    ...
    if stale_or_missing:
        warnings.append(
            ...
            "run `noeticweave review`."
        )
    if non_approved:
        warnings.append(
            f"review ledger: {non_approved} reviewed artifact(s) are not approved; run `noeticweave review`."
        )
    return info, warnings
...
def print_report(report: dict[str, object]) -> None:
    print(f"noeticweave doctor: {report['root']}")
