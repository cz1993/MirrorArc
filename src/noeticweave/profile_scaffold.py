...
from noeticweave.profiles import ProfileContract, profile_folder_paths
...
def render_profile_claude(profile: ProfileContract) -> str:
    ...
    return dedent(
        ...
        This NoeticWeave vault uses the `{profile.id}` profile (`{profile.profile_version}`).
        ...
        4. Run `python3.11 tools/noeticweave.py lint` before treating the vault as clean.
