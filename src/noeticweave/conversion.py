...
def load_source_records(root: Path) -> tuple[list[dict], list[str], list[str]]:
    ...
    if not path.exists():
        return [], [f"{SOURCE_MANIFEST.as_posix()}: missing; run `noeticweave sync` first."], []
    try:
    ...
    sections = [
        {
            "title": "Preflight",
            "items": [
                "Run `noeticweave status` and `noeticweave recovery` before sign-off.",
...
def validate_quality_results(
    ...
    *,
    ...
    if not (root / SOURCE_MANIFEST).exists():
        return summary, [], [f"{SOURCE_MANIFEST.as_posix()}: missing; run `noeticweave sync` first."]
...
def print_human(
    ...
    print(f"noeticweave conversion: {root}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Print a read-only NoeticWeave conversion spot-check report.")
