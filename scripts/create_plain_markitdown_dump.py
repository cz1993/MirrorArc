...
def render_dump(source_rel: Path, source_format: str, text: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "> Baseline-only conversion. No Vaultwright manifest identity, lifecycle state,",
...
def create_dump(
    ...
    *,
    ...
    if in_source_checkout(root) and not allow_source_checkout_root:
        raise ValueError("refusing to write private benchmark output inside the Vaultwright source checkout")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Allow writing under the Vaultwright source checkout; intended only for controlled local tests.",
    )
