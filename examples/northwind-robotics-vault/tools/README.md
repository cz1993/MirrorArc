...
| `noeticweave.py` | thin operator wrapper: `plan`, `sync`, `status`, `catalog`, `conversion`, `m365`, `migration`, `overlap`, `pilot`, `recovery`, `sandbox`, `lint`, `benchmark`, `doctor`, and repo-root `init` |
...
note with more inbound links when that signal is available. Use `noeticweave overlap` in copied
...
`tools/lint_vault.py` is a compatibility shim for the package-owned `noeticweave.lint` runtime.
Run it from an environment where NoeticWeave is installed, or from a source checkout with
`PYTHONPATH=src`.
...
python3.11 tools/noeticweave.py sync --json
python3.11 tools/noeticweave.py status
python3.11 tools/noeticweave.py status --json
python3.11 tools/noeticweave.py catalog
python3.11 tools/noeticweave.py catalog --html
python3.11 tools/noeticweave.py m365
python3.11 tools/noeticweave.py review --json
python3.11 tools/noeticweave.py overlap
python3.11 tools/noeticweave.py overlap --worksheet
...
python3.11 tools/noeticweave.py migration
python3.11 tools/noeticweave.py migration --worksheet
python3.11 tools/noeticweave.py migration --runbook
python3.11 tools/noeticweave.py migration --normalize-frontmatter-domains
python3.11 tools/noeticweave.py migration --normalize-frontmatter-domains --worksheet
python3.11 tools/noeticweave.py migration --normalize-frontmatter-domains --write
python3.11 tools/noeticweave.py pilot
...
python3.11 tools/noeticweave.py doctor
python3.11 tools/noeticweave.py doctor --json
```
...
```bash
python3.11 tools/noeticweave.py review --artifact CATALOG.html --status approved --reviewer CodeX
python3.11 tools/noeticweave.py review --artifact _mirrors/40_delivery/brief.md --status needs-work --reviewer Claude --note "Table spot-check needed"
python3.11 tools/noeticweave.py review --json
python3.11 tools/noeticweave.py review --check
```
...
It checks required NoeticWeave files/tools, verifies the copied vault is not the same path as the
...
`noeticweave.mirrors.office` runtime. Run it from an environment where NoeticWeave is installed, or
...
above-sentinel annotations, run `noeticweave migrate annotations --write` before refreshing it.
...
`noeticweave.mirrors.github_repos` runtime. Run it from an environment where NoeticWeave is
...
```bash
python3.11 tools/noeticweave.py benchmark
python3.11 tools/noeticweave.py benchmark --require-generated  # after running sync
```
...
`noeticweave_markdown`. The report prints per-mode scores, correction counts, privacy/provenance
