...
All notable changes to NoeticWeave are documented here. Format loosely follows
[Keep a Changelog](https://keepachangelog.com/); Python package prereleases use PEP 440.

## [Unreleased]

### Changed
- Renamed the project from Vaultwright to **NoeticWeave** after the preferred NoeticLens name
  failed exact-name collision screening; the distribution, canonical Python namespace, CLI,
  repository URLs, docs, templates, examples, benchmark mode, CI/release workflows, and local
  derived-state path now use the NoeticWeave identity.
- Kept narrow technical-alpha compatibility for the previous console command, Python namespace,
  ...
  new output and documentation use NoeticWeave.
...
- Marked experimental report/scaffold command surfaces in CLI help while preserving compatibility
  commands.
- Clarified the whitepaper's mirror-layer wording: NoeticWeave mirrors are manifest-backed,
...
- Added a package-version CLI smoke check (`noeticweave --version`) and wired it into
...
- Added package-first `uvx`/`pipx` onboarding guidance and updated `noeticweave init` next-step
  output to point beginners at the installed console command plus JSON pilot evidence checks.
- Added structured top-level `--json` output for `noeticweave sync`, `noeticweave status`, and
  `noeticweave doctor`, with CI/release smoke coverage for pilot-ready machine-readable evidence.
...
- Closed the Stage 2 V1-C3 official-profile gate: `noeticweave init --profile` now scaffolds
...
- Added optional Stage 1B native watch capture with `noeticweave watch --native`, backed by the
  `noeticweave[watch]` watchdog extra; native events are observed only under configured content
...
- Added Stage 1B watch startup orchestration with `noeticweave watch --once`, composing startup
  reconciliation, feed-event queueing, and journal replay; optional native capture is now handled
  by `noeticweave watch --native`.
- Added Stage 1B changed-file sync orchestration with `noeticweave sync --changed`, composing
  explicit reconciliation and journal replay while preserving the existing full-sync path as
  `noeticweave sync` / `noeticweave sync --full`.
- Added Stage 1B explicit reconciliation with `noeticweave reconcile`, queuing missed created,
...
- Added Stage 1B idempotent journal replay with `noeticweave journal replay`, recovering
...
- Added the first Stage 1B journal foundation: package-owned changed-file event/state modules,
  local `.noeticweave/state.sqlite` initialization, `noeticweave journal status`, `.noeticweave/`
...
- Added package-owned v1 profile contracts for `research-learning`, `software-project`, and
  `blank`; `noeticweave profile list/show` now exposes all four official profile contracts.
...
- The installable `noeticweave` CLI now supports `init --profile business-operations`,
  `profile list`, `profile show`, and `profile validate` against the package-owned profile
  contract.
- `noeticweave lint` and `noeticweave catalog` now read `_meta/profile.yml` for profile-defined
...
- The installable `noeticweave catalog` command now runs package-owned catalog code from
  `src/noeticweave/`, while the vault-local `tools/catalog_report.py` remains available as a
...
- `noeticweave overlap` now reads the active profile's domain folders when choosing curated notes
  for calibration, so profile-defined content roots participate without hard-coded business
  folders.
- `noeticweave overlap` now also reads profile-defined context frontmatter fields when counting
...
- `noeticweave catalog` now separates profile-defined machine-owned Markdown artifacts from curated
  Markdown/domain counts, while surfacing them as their own inventory metric.
- `noeticweave m365` now separates profile-defined machine-owned Markdown artifacts, including
  repo mirrors, from curated Markdown counts in handoff inventory.
- `noeticweave sandbox` now reports profile-defined machine-owned Markdown separately from curated
...
- Catalog, Microsoft 365 handoff, sandbox, doctor, review-ledger classification, and migration
  ...
  assuming `_mirrors/`.
- `noeticweave migration --worksheet`, `--runbook`, and frontmatter-normalization worksheets now
...
- `noeticweave pilot` now excludes the active Office mirror root from workspace inventory, so
  custom generated roots are not counted as operator content or source candidates.
- `noeticweave recovery` now excludes the active Office mirror root from missing-manifest source
...
- `noeticweave doctor` now reports profile-declared generated view health through the profile view
  planner, warning on missing or stale views without assuming every profile owns `Documents.base`.
- `noeticweave doctor` now validates the active profile contract first and treats
...
- `noeticweave lint` now treats a missing `_meta/domain-map.yml` as a non-blocking legacy-alias
...
- `noeticweave lint` now loads `_meta/profile.yml` through the package profile validator before
...
- `noeticweave lint` now reuses the shared runtime profile context helpers directly for context
  fields and aliases, preserving profile-less legacy fallback without local duplicate defaults.
- `noeticweave lint` now reuses the shared runtime repo-notes directory helpers for repo-mirror
...
- `noeticweave catalog` and `noeticweave migration` now load profile domain routing through the
...
- `noeticweave benchmark` and the aggregate `noeticweave pilot` report now load profile-declared
...
- Vault-local catalog, conversion, migration, overlap, benchmark, and pilot tools now also delegate
  ...
  behavior.
- The vault-local `tools/noeticweave.py` operator wrapper now delegates to the package CLI while
...
- `noeticweave lint` now blocks generated source/repo mirrors that still contain above-sentinel
...
- Office mirror planning, sync, and status behavior now lives in package-owned
  `noeticweave.mirrors.office`; vault-local `tools/sync_office_md.py` is a compatibility shim copied
  through the package template and examples.
- GitHub repo mirror planning, sync, and status behavior now lives in package-owned
  `noeticweave.mirrors.github_repos`; vault-local `tools/sync_github_repos.py` is a compatibility
  shim copied through the package template and examples.
- The installable `noeticweave plan`, `noeticweave sync`, and `noeticweave status` commands now
  orchestrate the package-owned Office and repo sync modules directly, while the vault-local
  `tools/noeticweave.py` wrapper remains available for compatibility.
- The installable `noeticweave doctor` command now runs package-owned preflight checks from
  `noeticweave.doctor`, including dependency, manifest, lifecycle-contract, recovery, review-ledger,
  Obsidian, backup, git, and GitHub auth posture checks.
- Vault health checks now live in package-owned `noeticweave.lint`; vault-local
  `tools/lint_vault.py` is a compatibility shim copied through the package template and examples.
- The installable `noeticweave conversion` command now runs package-owned `noeticweave.conversion`
...
- The installable `noeticweave m365` command now runs package-owned `noeticweave.m365` code;
...
- The installable `noeticweave migration` command now runs package-owned `noeticweave.migration`
...
- The installable `noeticweave overlap` command now runs package-owned `noeticweave.overlap` code;
...
- The installable `noeticweave benchmark` command now runs package-owned `noeticweave.benchmark`
  ...
  vault-local wrapper.
- The installable `noeticweave pilot` command now runs package-owned `noeticweave.pilot` code and
...
- The installable `noeticweave sandbox` command now runs package-owned `noeticweave.sandbox` code
...
- The installable `noeticweave recovery` command now runs package-owned `noeticweave.recovery`
...
- The installable `noeticweave review` command now runs package-owned `noeticweave.review_ledger`
...
- Added `tools/catalog_report.py` and `noeticweave catalog`, which writes a generated
...
- `noeticweave catalog --html` now writes the same path-and-metadata-only inventory as a static
  `CATALOG.html` gateway with aggregate charts for reviewers who prefer a browser surface.
- `noeticweave catalog` now surfaces lifecycle contract provenance from source/repo manifests,
...
- Added `docs/MICROSOFT_365_HANDOFF.md` and `noeticweave m365`, a read-only handoff readiness
  report for Microsoft 365, SharePoint, OneDrive, Copilot Studio, and connector review paths.
- Updated the Microsoft 365 handoff and whitepaper guidance to treat NoeticWeave as a Copilot
...
- Added `tools/review_ledger.py` and `noeticweave review`, a metadata-only review ledger that
...
- `noeticweave pilot` now summarizes review-ledger aggregate counts, including reviewed artifacts,
  stale/missing reviews, and non-approved decisions, without exposing artifact paths or notes.
- `noeticweave pilot` now also summarizes overlap-calibration aggregate counts and current
  thresholds, without exposing note bodies, shared terms, source text, paths, or reviewer notes.
- `noeticweave doctor` now reports review-ledger approval posture and warns on stale/missing or
  non-approved artifact reviews without printing artifact paths, reviewer names, or notes.
- `noeticweave migration` now reports legacy or unknown note frontmatter domains using
  `_meta/domain-map.yml` aliases, giving operators a read-only cleanup queue before moving notes.
- `noeticweave migration --worksheet` now prints a Markdown review checklist for legacy folder and
  frontmatter-domain cleanup batches.
- `noeticweave migration --normalize-frontmatter-domains` now previews known legacy frontmatter
...
- `noeticweave migration --normalize-frontmatter-domains --worksheet` now prints a dry-run review
...
- `noeticweave migration --runbook` now prints a read-only legacy folder migration protocol with
...
- `noeticweave recovery --worksheet` now prints a read-only Markdown recovery checklist with
...
- `noeticweave recovery --runbook` now prints a read-only, state-grouped resolution protocol for
...
- CI and release smoke checks now exercise recovery worksheet and runbook output through the
  packaged CLI.
- Added `tools/sandbox_report.py` and `noeticweave sandbox`, a read-only copied-vault preflight for
...
- The installable `noeticweave` console entry point now exercises `sandbox` through the package
...
- `noeticweave benchmark` now reports citation counts and supports `--require-citations` so scored
  agent-readiness results can be gated on declared source or generated-mirror evidence.
- `noeticweave benchmark` now tracks prompt-safety review and prompt-safety violations in private
...
- `noeticweave benchmark --init-tasks` now writes a private task-pack scaffold from synced
...
- `noeticweave benchmark --init-results` now writes a private, fillable result-pack scaffold for
...
- `noeticweave benchmark --worksheet` now prints a private Markdown run sheet from the task pack,
...
- `noeticweave conversion --init-results` now creates private metadata-only conversion-quality
  result scaffolds, `noeticweave conversion --results ... --require-reviewed` validates
  reviewer-entered status/score/correction/issue-code aggregates, `noeticweave pilot` summarizes
...
- `noeticweave recovery` now compares `_meta/repo-manifest.json` with `tools/repos.yml` so removed
  repo config entries are surfaced as `repo_unconfigured` before another repo sync rewrites the
  manifest.
- `noeticweave catalog` and `noeticweave m365` now use the same repo config comparison, so
...
- `noeticweave lint` now blocks repo manifest records that are no longer governed by
...
- Added `_meta/lifecycle-states.yml` as the machine-readable Office/repo lifecycle contract, and
  `noeticweave doctor` now validates that every state has entry conditions, explanations, permitted
...
- Lint now reports legacy frontmatter domain aliases with canonical recommendations, for example
  `marketing -> market (20_market/)`, so copied-vault cleanup queues align with `noeticweave
  migration`.
- Added `tools/overlap_report.py` and `noeticweave overlap`, a read-only overlap-threshold
...
- `noeticweave sandbox` no longer treats `_meta/domain-map.yml` and `_meta/mirror-config.yml` as
...
- Decide CLA vs DCO; secure the "NoeticWeave" name; draft the commercial agreement.
...
- Added `docs/NOETICWEAVE_WHITEPAPER.md`, a professional review whitepaper covering progress,
  limitations, validation posture, dogfood results, and future roadmap.
- Updated the whitepaper to frame NoeticWeave's future direction as an agent-ready markdown
...
- Added `tools/benchmark_tasks.py` and `noeticweave benchmark` to validate agent-readiness task
  packs before and after generated mirrors exist.
- `noeticweave benchmark` now validates optional `_meta/agent-readiness-results.yml` result packs
...
- Added `tools/noeticweave.py`, a thin operator wrapper for `plan`, `sync`, `status`, `lint`, and
  `doctor`, plus repo-root `init`.
- Added `pyproject.toml` and the source-installable `noeticweave` console entry point; current
  command behavior has since converged into package-owned modules while vault-local tools remain
  compatibility shims.
- Packaged the starter vault template under `src/noeticweave/template`, so `noeticweave init` can
  scaffold from an installed wheel without a source checkout or `NOETICWEAVE_REPO`.
- CI now builds a wheel, installs it into a clean environment, and smoke-tests packaged
  `noeticweave init`, `doctor`, `plan`, `benchmark`, `conversion`, `migration`, `pilot`, and JSON
  `recovery` delegation.
...
- `noeticweave doctor` now reports manifest lifecycle counts, sync audit presence, recovery action
...
- Added `tools/recovery_report.py` and `noeticweave recovery`, a read-only recovery checklist for
  non-clean source/repo manifest records and missing generated paths.
- Added `tools/conversion_report.py` and `noeticweave conversion`, a read-only conversion
...
- `noeticweave conversion --guide` now appends a manifest-aware operator checklist for conversion
...
- Added `tools/migration_report.py` and `noeticweave migration`, a read-only report for legacy
  alias folders and unknown top-level folders, including non-reserved hidden/underscore folders,
  before any manual migration.
- Added `tools/pilot_report.py`, `noeticweave pilot`, and `docs/PILOT_WORKSHEET.md` for aggregate
  design-partner evidence capture without printing source or mirror content.
- Added `noeticweave pilot --worksheet`, a redacted Markdown summary mode for private pilot records
...
- CI now smoke-tests the installed `noeticweave` console entry point and validates both example
