...
- Replaced the two legacy example vaults with the 50+ file Ontario electricity evidence workspace:
  ...
  metadata-only references; the corpus contains no company attribution or forward-looking output.
...
- Removed the pre-release Vaultwright console and Python-package aliases; local-state and benchmark
  migration readers remain narrowly compatible so existing technical-alpha vaults can be upgraded.
