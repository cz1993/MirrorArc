...
NoeticWeave is mostly *discipline*, encoded so an agent follows it. This is the why behind the
...
This is NoeticWeave's core technical idea and its main differentiator — see `positioning.md`.
...
So NoeticWeave optimizes for *fewer, better-connected, current* notes:
...
- **Lint** — periodically check frontmatter, links, orphans, overlap candidates, mirror gaps, and
  ...
  `noeticweave overlap` before tuning overlap sensitivity in copied pilot vaults through
