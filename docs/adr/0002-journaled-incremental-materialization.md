...
**Status:** Accepted
**Date:** 2026-06-24
**Decision source:** `docs/MIRRORARC_WHITEPAPER.md`
**Owner:** cz1993

## Context

MirrorArc already has package-owned full sync, source/repo manifests, machine-owned mirrors,
...
source file before MirrorArc can update the one affected mirror.
...
temporary lock files or atomic replacement patterns. MirrorArc therefore cannot claim literal WAL
shipping.
...
MirrorArc v1 adds a mandatory Stage 1B requirement: **journaled changed-file materialization**.
...
The local journal uses a private derived-state database such as `.mirrorarc/state.sqlite`. It may
...
Before computing a full source hash, MirrorArc compares cheap metadata:
...
source body. If the fingerprint changed, MirrorArc computes SHA-256. If the SHA-256 is unchanged,
...
Watcher delivery is advisory. MirrorArc must reconcile at startup, on explicit command, and on a
...
ignore `.mirrorarc/`, run full sync, and rebuild derived state from sources and manifests.
