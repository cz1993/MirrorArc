...
NoeticWeave is a local/open-core document governance tool. It is not currently a hosted service and
...
NoeticWeave must document whether a workflow sends source or mirror content to a cloud model. The
...
NoeticWeave must treat source text as untrusted input. Future AI workflows should separate "source
...
- Obsidian community plugins are outside NoeticWeave's trust boundary.
- `noeticweave doctor` reports whether optional Obsidian config and community plugins are present,
...
Run `noeticweave sandbox --source-root <original-source-root>` from copied pilot vaults before the
...
- Cloud sync tools may create conflict files or expose data outside NoeticWeave's control.
