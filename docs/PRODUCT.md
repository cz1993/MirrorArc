...
Current execution remains evidence-first: run representative corpora through the core mirror,
catalog, benchmark, recovery, and handoff flow before adding indexes or connectors that could hide
weak provenance. The Ontario electricity example supplies a public, reproducible demonstration corpus;
private validation evidence stays outside this repository.

The interactive UI generated by `catalog --html` remains inside that boundary. Its safe default is
a portable, self-contained presentation of path and manifest metadata already produced by the
catalog command. For deliberate local review, `--include-content` may add bounded Markdown and
generated-mirror bodies to the same file; the output then inherits the vault's sensitivity and must
not be treated as a share-safe catalog. Both modes pin `INDEX.md` as the beginner entry point,
provide a resizable catalog with readable filenames, separate relationship, metadata, and
rendered-content views, and keep context-pack downloads metadata-only. They do not
add a local evidence index, vector store, hosted service, or connector. The broader indexed Explorer
remains gated by the validation-first execution order.

The first paid workflow remains consulting and implementation work, but the v1 architecture is no
longer a single business-operations folder template. Profiles are versioned contracts for domain
vocabulary, note types, lifecycle states, folders, policies, generated views, and benchmark tasks;
the contract is documented in [`docs/PROFILE_SCHEMA.md`](PROFILE_SCHEMA.md), while source and
materialization authority are documented in [`docs/SYNC_SPEC.md`](SYNC_SPEC.md).
...
- repo mirrors under the active profile's `repo_notes_dir` (`20_sources/repos/` in the default
  `data-product` profile);
- generated `CATALOG.md` inventory and a self-contained, metadata-only `CATALOG.html` Catalog
  Explorer;
...
more than a thesis. Private task and result packs must stay outside the public repository.
