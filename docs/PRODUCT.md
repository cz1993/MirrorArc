...
MirrorArc turns heterogeneous source collections into governed, profile-driven knowledge
...
MirrorArc's first buyer is a small consulting, advisory, or implementation team that handles
...
1. Point MirrorArc at an existing client document collection.
...
- Local filesystem source; cloud-synced or mounted/network folders are acceptable only when files
  are pinned locally, backed up, and checked with `mirrorarc doctor` before production sync.
...
properties, graph views, and Bases. MirrorArc correctness must not depend on Obsidian sync,
...
MirrorArc's long-term value depends on whether agents perform better against governed markdown
