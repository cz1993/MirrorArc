...
| `plain_markitdown_dump` | One-off Markdown conversion baseline without MirrorArc manifests, lifecycle state, or curated mirror workflow. |
| `mirrorarc_markdown` | MirrorArc task context: source-linked curated notes plus generated mirror workflow and manifest/lifecycle semantics. |
...
| `mirrorarc_markdown` | 6 | 12 / 12 | 2.00 | 0 | 0 | 6 / 6 |
...
mirrorarc_markdown: results=6 score=12/12 avg=2.00 corrections=0 violations=0
```
...
vault. `_benchmark/` is ignored by the MirrorArc template because it may contain private run
...
mirrorarc --root /tmp/mirrorarc-messy-benchmark sync --json
mirrorarc --root /tmp/mirrorarc-messy-benchmark benchmark \
...
mirrorarc_markdown: results=5 score=10/10 avg=2.00 corrections=0 violations=0
```
