...
Create the vault without cloning NoeticWeave itself. The installable command requires Python 3.11+;
...
`software-project`, and `blank`. Each initializes through `noeticweave init --profile <profile-id>`;
...
noeticweave --root ~/my-business-vault sandbox --source-root /path/to/original-documents # copied-vault preflight
noeticweave --root ~/my-business-vault plan          # inspect source inventory and proposed mirrors
noeticweave --root ~/my-business-vault sync          # mirrors -> _mirrors/ and profile repo_notes_dir
noeticweave --root ~/my-business-vault sync --json   # machine-readable sync evidence
...
noeticweave --root ~/my-business-vault catalog       # write CATALOG.md inventory gateway
noeticweave --root ~/my-business-vault catalog --html # write CATALOG.html visual inventory gateway
noeticweave --root ~/my-business-vault m365          # Microsoft 365/Copilot handoff readiness
noeticweave --root ~/my-business-vault review --json # summarize metadata-only review decisions
noeticweave --root ~/my-business-vault overlap       # calibrate overlap thresholds without note bodies
noeticweave --root ~/my-business-vault conversion --guide # read-only conversion spot-check and guide
noeticweave --root ~/my-business-vault conversion --init-results # private quality result scaffold
...
noeticweave --root ~/my-business-vault migration --worksheet # Markdown cleanup checklist
noeticweave --root ~/my-business-vault migration --runbook # legacy folder move protocol
noeticweave --root ~/my-business-vault migration --normalize-frontmatter-domains --worksheet # domain cleanup checklist
noeticweave --root ~/my-business-vault recovery --worksheet # manifest recovery checklist
...
noeticweave --root ~/my-business-vault benchmark     # validate benchmark tasks, if configured
...
- **Need a non-Obsidian gateway?** Regenerate `CATALOG.md` with
  `noeticweave --root ~/my-business-vault catalog`, or `CATALOG.html` with
  `noeticweave --root ~/my-business-vault catalog --html`; both list source paths, mirrors,
...
- **Reviewed an artifact?** Record the decision with
  `noeticweave --root ~/my-business-vault review --artifact CATALOG.html --status approved --reviewer <name>`.
  The ledger stores hashes and short metadata notes only, then reports approvals as stale if the
  reviewed artifact changes.
- **Housekeeping?** Ask it to *lint* — or just run `noeticweave --root ~/my-business-vault lint`.
...
- **Agent-readiness pilot?** Use `docs/AGENT_READINESS_BENCHMARK.md` and
  `noeticweave --root ~/my-business-vault benchmark --init-tasks` to create a private task scaffold
  after sync, then `noeticweave --root ~/my-business-vault benchmark --worksheet` to run the
  comparison, then `noeticweave --root ~/my-business-vault benchmark --init-results` and
  `noeticweave --root ~/my-business-vault benchmark --results _meta/agent-readiness-results.yml`
  to compare raw-source, plain markitdown dump, and NoeticWeave-markdown performance on the same questions. Add
...
- **Microsoft 365 handoff?** Use `docs/MICROSOFT_365_HANDOFF.md` and
  `noeticweave --root ~/my-business-vault m365` to check whether the generated mirror/catalog layer
