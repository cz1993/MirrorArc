...
NoeticWeave is still a technical alpha. Releases should be conservative, source-backed, and honest
...
5. Review `CHANGELOG.md`, `README.md`, and `docs/NOETICWEAVE_WHITEPAPER.md` for overclaims.
...
Use a PEP 440-compatible alpha tag that matches the package version while NoeticWeave is alpha:
...
```bash
gh run list --repo cz1993/noeticweave --workflow Release --limit 5
gh run watch <run-id> --repo cz1993/noeticweave --exit-status
```
...
- confirm `noeticweave init` works from the released wheel;
