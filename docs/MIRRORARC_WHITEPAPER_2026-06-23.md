...
> `docs/MIRRORARC_WHITEPAPER.md`, `docs/V1_FINISH_LINE.md`, `docs/VALIDATION_GATE.md`,
...
**Date:** 2026-06-23
**Repository reviewed:** `cz1993/MirrorArc` at commit `2b9e79823f1ebac4b123fc434e478f82bb8d6c87`
...
MirrorArc should evolve from a business-specific document-vault template into a **profile-driven, governed knowledge workspace compiler**.
...
MirrorArc can become domain-agnostic at the product-architecture level while retaining a narrow commercial wedge. The current business-operations workflow should remain the flagship paid implementation path until other profiles show comparable evidence of value.
...
> **MirrorArc turns heterogeneous source collections into governed, profile-driven knowledge workspaces that humans and AI agents can inspect, navigate, cite, and refresh without replacing the original records.**
...
“Any type of knowledge base” is too broad to be useful. MirrorArc should target **source-backed knowledge workspaces** with most of these characteristics:
...
MirrorArc should not optimize for every note-taking use case. It does not need to replace journaling apps, lightweight personal notes, creative writing tools, task managers, or generic bookmark managers.

This boundary preserves differentiation. A simple reading list with no source lifecycle may not need MirrorArc. A literature-review workspace with PDFs, citations, claims, annotations, evolving synthesis notes, and agent-assisted retrieval does.
...
The `kepano/obsidian-skills` project is an excellent interoperability reference. It provides Agent Skills for Obsidian-flavored Markdown, Bases, JSON Canvas, the Obsidian CLI, and web extraction. These skills teach agents how to produce valid Obsidian artifacts; they do not provide MirrorArc's provenance, lifecycle, review, or safety model.

MirrorArc should integrate at three levels.

### 6.1 Use, do not duplicate, syntax expertise

MirrorArc should declare compatibility with the Obsidian skills and provide an installer or setup report rather than copying their entire content by default.

```text
mirrorarc obsidian doctor
mirrorarc obsidian install-skills --agent codex
mirrorarc obsidian install-skills --agent claude
```

Pin the tested skill version in a lock file and record attribution. Because the referenced project is MIT-licensed, vendoring is possible, but external installation is preferable until MirrorArc needs a controlled fork.

### 6.2 Add MirrorArc-specific governance skills
...
Obsidian remains an optional presentation adapter. MirrorArc correctness must remain testable without opening Obsidian.
...
CodeGraph itself is specialized for source code. It extracts symbols and call edges using language parsers. MirrorArc should not fork that implementation into a general-document product.

### 7.1 Evidence-index model

MirrorArc's derived index should contain nodes such as:
...
mirrorarc explore "How is this decision supported?"
```
...
For a software-project profile, MirrorArc may query CodeGraph through its CLI or MCP interface for symbol, dependency, and impact context. MirrorArc should preserve the higher-level evidence chain between code, documentation, ADRs, releases, and incidents while CodeGraph owns code-level semantic analysis.
...
MirrorArc should not clone a code-centric native macOS application. It should build a cross-platform, read-only Explorer over its own profile and evidence models.
...
mirrorarc.explore
mirrorarc.renderers
mirrorarc.adapters.obsidian
mirrorarc.adapters.codegraph
mirrorarc.security
```
...
3. MirrorArc profile plus catalog;
4. MirrorArc profile plus evidence index;
5. MirrorArc profile plus evidence index and curated knowledge.
...
| CodeGraph duplication | Medium | Use an adapter for code semantics; keep MirrorArc focused on evidence lifecycle |
...
- move runtime logic into `src/mirrorarc/`;
...
- five MirrorArc governance skills;
...
- one MCP tool, `mirrorarc_explore`;
...
MirrorArc v1 Core is finished when all of the following exist and pass their gates:
...
MirrorArc should not remain permanently tied to finance, HR, legal, and startup operations. Those concepts belong in one profile. The core opportunity is a governed compiler for source-backed knowledge workspaces.
...
- MirrorArc current whitepaper: https://github.com/cz1993/MirrorArc/blob/main/docs/MIRRORARC_WHITEPAPER.md
