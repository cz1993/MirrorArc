...
MirrorArc sync must prove the core product promise: source files remain untouched while generated
...
- implemented for Stage 1B Office materialization: `mirrorarc.changes.materialize` can process
...
- implemented for Stage 1B candidate stability: `mirrorarc.changes.stability` waits for the
...
- implemented for Stage 1B current-path Office worker processing:
  `mirrorarc.changes.worker` acquires the workspace lease, claims queued/ready journal events,
  materializes supported current-path Office sources through `mirrorarc.changes.materialize`,
...
- implemented for Stage 1B deleted-source lifecycle replay:
  ...
  `mirrorarc.changes.materialize.materialize_office_delete`, marking the Office source manifest
...
- implemented for Stage 1B journal replay: `mirrorarc.changes.replay` and
  `mirrorarc journal replay` recover interrupted `processing` events, optionally requeue failed
...
- implemented for Stage 1B explicit reconciliation: `mirrorarc.changes.reconcile` and
  `mirrorarc reconcile` discover current Office/PDF candidates through the existing mirror
...
- implemented for Stage 1B changed sync: `mirrorarc.changes.changed_sync` and
  `mirrorarc sync --changed` run explicit reconciliation and then replay claimable journal work
  through the existing worker/materialization path; plain `mirrorarc sync` remains the existing
  full Office/repo sync path, and `mirrorarc sync --full` names that recovery path explicitly;
- implemented for Stage 1B watch startup orchestration: `mirrorarc.changes.watch` and
  `mirrorarc watch --once` run startup reconciliation, queue normalized/coalesced feed events
  through the existing change-feed interface, and replay claimable journal work under the existing
  worker lease; plain `mirrorarc watch` exits with mode guidance;
- implemented for Stage 1B optional native watch capture: `mirrorarc.changes.native_watch` maps
...
- implemented for mirror-annotation migration: `mirrorarc migrate annotations --plan` reports
...
- implemented for lifecycle operator semantics: the template now includes
  ...
  state, `mirrorarc doctor` validates the contract during preflight, and tests ensure sync/report
...
If an OS/process interruption leaves a hidden atomic temp file behind, `mirrorarc recovery` must
