...
NoeticWeave sync must prove the core product promise: source files remain untouched while generated
...
candidate-only hashing for safe move detection. `noeticweave sync --changed` composes
reconciliation and replay, while `noeticweave sync` and `noeticweave sync --full` preserve the full
sync recovery path. `noeticweave watch --once` runs the deterministic watch-start cycle: startup
reconciliation, feed-event queueing, and journal replay. `docs/JOURNALED_MATERIALIZATION_BENCHMARK.md`
records synthetic known-path replay evidence. `noeticweave watch --native` provides optional
...
- implemented for Stage 1B Office materialization: `noeticweave.changes.materialize` can process
...
- implemented for Stage 1B candidate stability: `noeticweave.changes.stability` waits for the
...
- implemented for Stage 1B current-path Office worker processing:
  `noeticweave.changes.worker` acquires the workspace lease, claims queued/ready journal events,
  materializes supported current-path Office sources through `noeticweave.changes.materialize`,
...
- implemented for Stage 1B deleted-source lifecycle replay:
  ...
  `noeticweave.changes.materialize.materialize_office_delete`, marking the Office source manifest
...
- implemented for Stage 1B journal replay: `noeticweave.changes.replay` and
  `noeticweave journal replay` recover interrupted `processing` events, optionally requeue failed
...
- implemented for Stage 1B explicit reconciliation: `noeticweave.changes.reconcile` and
  `noeticweave reconcile` discover current Office/PDF candidates through the existing mirror
...
- implemented for Stage 1B changed sync: `noeticweave.changes.changed_sync` and
  `noeticweave sync --changed` run explicit reconciliation and then replay claimable journal work
  through the existing worker/materialization path; plain `noeticweave sync` remains the existing
  full Office/repo sync path, and `noeticweave sync --full` names that recovery path explicitly;
- implemented for Stage 1B watch startup orchestration: `noeticweave.changes.watch` and
  `noeticweave watch --once` run startup reconciliation, queue normalized/coalesced feed events
  through the existing change-feed interface, and replay claimable journal work under the existing
  worker lease; plain `noeticweave watch` exits with mode guidance;
- implemented for Stage 1B optional native watch capture: `noeticweave.changes.native_watch` maps
  ...
  and `noeticweave watch --native` flushes captured events through the same reconciliation, feed,
  coalescing, lease, replay, and source-addressable materialization path; the optional
  `noeticweave[watch]` extra supplies the watchdog dependency without changing default installs;
...
- implemented for mirror-annotation migration: `noeticweave migrate annotations --plan` reports
...
- implemented for lifecycle operator semantics: the template now includes
  ...
  state, `noeticweave doctor` validates the contract during preflight, and tests ensure sync/report
...
If an OS/process interruption leaves a hidden atomic temp file behind, `noeticweave recovery` must
