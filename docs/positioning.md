...
MirrorArc's underlying pattern is **not novel**, and pretending otherwise would waste your time.
This page is the candid map of the landscape and where MirrorArc actually earns its place.
...
- **Copilot for Obsidian** (~7k★) — the closest *product*; its v4 (summer 2026) brings Claude
  Code/Codex into the vault. A general harness, not an opinionated methodology — MirrorArc could
  run *on top of* it.
...
## Where MirrorArc is differentiated
...
- **Sits alongside** basic-memory or Copilot — they can chat/recall; MirrorArc keeps the curated,
...
- **Cheap to clone.** Instruction/skill files copy trivially — which is exactly why MirrorArc
