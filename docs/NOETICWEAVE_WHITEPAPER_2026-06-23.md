# NoeticWeave Whitepaper

> Historical snapshot. This file is not the current execution authority. Use
> `docs/NOETICWEAVE_WHITEPAPER.md`, `docs/V1_FINISH_LINE.md`, `docs/VALIDATION_GATE.md`,
...
**Date:** 2026-06-23
**Repository reviewed:** `cz1993/noeticweave` at commit `2b9e79823f1ebac4b123fc434e478f82bb8d6c87`
...
NoeticWeave should evolve from a business-specific document-vault template into a **profile-driven, governed knowledge workspace compiler**.
...
NoeticWeave can become domain-agnostic at the product-architecture level while retaining a narrow commercial wedge. The current business-operations workflow should remain the flagship paid implementation path until other profiles show comparable evidence of value.
...
> **NoeticWeave turns heterogeneous source collections into governed, profile-driven knowledge workspaces that humans and AI agents can inspect, navigate, cite, and refresh without replacing the original records.**
...
“Any type of knowledge base” is too broad to be useful. NoeticWeave should target **source-backed knowledge workspaces** with most of these characteristics:
...
NoeticWeave should not optimize for every note-taking use case. It does not need to replace journaling apps, lightweight personal notes, creative writing tools, task managers, or generic bookmark managers.

This boundary preserves differentiation. A simple reading list with no source lifecycle may not need NoeticWeave. A literature-review workspace with PDFs, citations, claims, annotations, evolving synthesis notes, and agent-assisted retrieval does.
...
```text
noeticweave init --profile business-operations <path>
noeticweave profile list
noeticweave profile show
noeticweave profile validate
noeticweave profile diff <target-profile-version>
noeticweave profile migrate --plan
noeticweave profile migrate --write
noeticweave profile views --check
noeticweave profile views --write
```
...
The `kepano/obsidian-skills` project is an excellent interoperability reference. It provides Agent Skills for Obsidian-flavored Markdown, Bases, JSON Canvas, the Obsidian CLI, and web extraction. These skills teach agents how to produce valid Obsidian artifacts; they do not provide NoeticWeave's provenance, lifecycle, review, or safety model.

NoeticWeave should integrate at three levels.

### 6.1 Use, do not duplicate, syntax expertise

NoeticWeave should declare compatibility with the Obsidian skills and provide an installer or setup report rather than copying their entire content by default.

```text
noeticweave obsidian doctor
noeticweave obsidian install-skills --agent codex
noeticweave obsidian install-skills --agent claude
```

Pin the tested skill version in a lock file and record attribution. Because the referenced project is MIT-licensed, vendoring is possible, but external installation is preferable until NoeticWeave needs a controlled fork.

### 6.2 Add NoeticWeave-specific governance skills
...
- `noeticweave-explore`;
- `noeticweave-review`;
- `noeticweave-consolidate`;
- `noeticweave-visualize`.
...
CodeGraph itself is specialized for source code. It extracts symbols and call edges using language parsers. NoeticWeave should not fork that implementation into a general-document product.

### 7.1 Evidence-index model

NoeticWeave's derived index should contain nodes such as:
...
```text
noeticweave index build
noeticweave index status
noeticweave explore "How is this decision supported?"
```
...
For a software-project profile, NoeticWeave may query CodeGraph through its CLI or MCP interface for symbol, dependency, and impact context. NoeticWeave should preserve the higher-level evidence chain between code, documentation, ADRs, releases, and incidents while CodeGraph owns code-level semantic analysis.
...
NoeticWeave should not clone a code-centric native macOS application. It should build a cross-platform, read-only Explorer over its own profile and evidence models.
...
```text
noeticweave migrate annotations --plan
noeticweave migrate annotations --write
```
...
```text
noeticweave.core
noeticweave.profiles
noeticweave.sources
noeticweave.mirrors
noeticweave.lifecycle
noeticweave.index
noeticweave.explore
noeticweave.renderers
noeticweave.adapters.obsidian
noeticweave.adapters.codegraph
noeticweave.security
```
...
3. NoeticWeave profile plus catalog;
4. NoeticWeave profile plus evidence index;
5. NoeticWeave profile plus evidence index and curated knowledge.
...
| CodeGraph duplication | Medium | Use an adapter for code semantics; keep NoeticWeave focused on evidence lifecycle |
...
- move runtime logic into `src/noeticweave/`;
...
- five NoeticWeave governance skills;
...
- one MCP tool, `noeticweave_explore`;
...
NoeticWeave v1 Core is finished when all of the following exist and pass their gates:
...
NoeticWeave should not remain permanently tied to finance, HR, legal, and startup operations. Those concepts belong in one profile. The core opportunity is a governed compiler for source-backed knowledge workspaces.
...
- NoeticWeave repository: https://github.com/cz1993/noeticweave
- NoeticWeave current whitepaper: https://github.com/cz1993/noeticweave/blob/main/docs/NOETICWEAVE_WHITEPAPER.md
