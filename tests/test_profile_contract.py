...
from mirrorarc.profile_migration import target_dir_paths
from mirrorarc.profiles import (
    ProfileContract,
...
def run_cli(*args: str, cwd: Path | None = None) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", *args],
        cwd=cwd or ROOT,
        env=env,
        text=True,
        capture_output=True,
...
def test_documents_base_matches_profile_generated_view() -> None:
    vaults = [
        ...
        ROOT / "examples" / "ontario-electricity-evidence-vault",
    ]
...
def test_packaged_and_example_profiles_match_template() -> None:
    ...
    profile_paths = [
        ...
        ROOT / "examples" / "ontario-electricity-evidence-vault" / "_meta" / "profile.yml",
    ]
...
def test_profile_cli_initializes_and_validates_current_profile(tmp_path: Path) -> None:
    ...
    assert "python3.11 tools/mirrorarc.py" not in init.stdout
