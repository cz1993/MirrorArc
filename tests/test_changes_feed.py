...
from mirrorarc.changes import feed, fingerprint, journal


def journal_rows(root: Path) -> list[sqlite3.Row]:
    conn = sqlite3.connect(root / ".mirrorarc" / "state.sqlite")
...
def test_feed_filters_generated_state_operational_and_temporary_paths(tmp_path: Path) -> None:
    ...
    changes = [
        ...
        feed.ObservedChange("modified", ".mirrorarc/state.sqlite"),
