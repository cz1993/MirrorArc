...
from noeticweave.changes import journal
...
def test_journal_status_reports_uninitialized_without_creating_state(tmp_path: Path) -> None:
    ...
    assert payload["state_path"] == ".noeticweave/state.sqlite"
...
def test_journal_initializes_sqlite_state_in_derived_directory(tmp_path: Path) -> None:
    ...
    assert path == tmp_path / ".noeticweave" / "state.sqlite"
    assert path.exists()
    payload = journal.journal_status(tmp_path)
...
def test_journal_reuses_legacy_state_without_creating_a_second_database(tmp_path: Path) -> None:
    ...
    (tmp_path / ".noeticweave").rename(tmp_path / ".vaultwright")
...
def test_journal_rejects_invalid_event_values_and_unsafe_paths(tmp_path: Path) -> None:
    ...
    with pytest.raises(journal.JournalError, match="derived state"):
        journal.record_event(tmp_path, "modified", current_path=".noeticweave/state.sqlite")
    with pytest.raises(journal.JournalError, match="derived state"):
...
def test_journal_cli_status_init_creates_state(tmp_path: Path) -> None:
    ...
    assert payload["initialized"] is True
    assert payload["schema_version"] == 1
    assert (tmp_path / ".noeticweave" / "state.sqlite").exists()


def test_journal_status_migrates_older_schema_metadata(tmp_path: Path) -> None:
    journal.initialize(tmp_path)
    with sqlite3.connect(tmp_path / ".noeticweave" / "state.sqlite") as conn:
...
def test_journal_status_reports_future_schema_without_hard_error(tmp_path: Path) -> None:
    ...
    assert "newer than this NoeticWeave supports" in payload["warnings"][0]
    assert result.returncode == 0, result.stderr
