...
def test_mirrorarc_cli_doctor_passes_on_template() -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "template/tools/mirrorarc.py"), "doctor"],
        cwd=ROOT / "template",
        text=True,
        capture_output=True,
    )
    ...
    assert "mirrorarc doctor: OK" in result.stdout
...
def test_mirrorarc_cli_doctor_reports_manifest_lifecycle_counts(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "doctor"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_cli_doctor_reports_review_ledger_posture_without_details(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    ...
    assert "warning: review ledger: 1 reviewed artifact(s) are stale, missing, or unreadable; run `mirrorarc review`." in result.stdout
    assert "warning: review ledger: 1 reviewed artifact(s) are not approved; run `mirrorarc review`." in result.stdout
...
def test_mirrorarc_cli_doctor_reports_obsidian_and_backup_posture(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
...
def test_mirrorarc_cli_doctor_reports_profile_neutral_nested_git_boundary(tmp_path: Path) -> None:
...
def test_mirrorarc_cli_doctor_reports_missing_profile_declared_view(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    ...
    assert result.returncode == 0, result.stderr or result.stdout
    assert "warning: profile view: Documents.base missing; run `mirrorarc profile views --write`." in result.stdout
    assert "Obsidian Bases index: Documents.base missing" not in result.stdout


def test_mirrorarc_cli_doctor_does_not_assume_documents_base_when_profile_omits_views(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
...
def test_mirrorarc_cli_doctor_uses_profile_defaults_without_legacy_alias_files(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
...
def test_mirrorarc_cli_doctor_requires_legacy_files_without_profile(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
...
def test_mirrorarc_cli_doctor_fails_invalid_profile_contract(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
...
def test_mirrorarc_cli_doctor_does_not_trust_commented_gitignore_patterns(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
...
def test_mirrorarc_cli_doctor_flags_negated_gitignore_patterns(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    (vault / ".gitignore").write_text(
        "data/\n"
...
def test_mirrorarc_cli_doctor_handles_unreadable_obsidian_json(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    obsidian = vault / ".obsidian"
    obsidian.mkdir()
...
def test_mirrorarc_cli_root_uses_target_vault_tools(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    ...
    result = subprocess.run(
        [sys.executable, str(ROOT / "template/tools/mirrorarc.py"), "--root", str(vault), "plan"],
        cwd=ROOT,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_cli_wrapper_defaults_to_own_vault_root(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr or result.stdout
    assert f"mirrorarc doctor: {vault.resolve()}" in result.stdout
    assert "mirrorarc doctor: OK" in result.stdout
...
def test_packaged_plan_sync_status_do_not_require_vault_wrapper(tmp_path: Path) -> None:
    ...
    for command in ("plan", "sync", "status"):
        result = subprocess.run(
            [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), command],
            cwd=ROOT,
            env=env,
            text=True,
            capture_output=True,
    ...
    assert "mirrorarc plan: no tools/repos.yml found; repo plan skipped" in results["plan"].stdout
    ...
    assert "mirrorarc status: no tools/repos.yml found; repo status skipped" in results["status"].stdout
...
def test_packaged_sync_status_doctor_json_outputs_are_structured(tmp_path: Path) -> None:
    ...
    doctor = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "doctor", "--json"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    status = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "status", "--json"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    sync = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "sync", "--json"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
...
def test_packaged_review_does_not_require_vault_wrapper(tmp_path: Path) -> None:
    ...
    check = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "review", "--check"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert "missing tools/mirrorarc.py" not in check.stderr
...
def test_packaged_recovery_does_not_require_vault_wrapper_or_local_report(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "recovery"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    worksheet = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "recovery", "--worksheet"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert "# MirrorArc Recovery Worksheet" in worksheet.stdout
...
def test_packaged_m365_does_not_require_vault_wrapper_or_local_report(tmp_path: Path) -> None:
    ...
    assert "Run `mirrorarc sync` before handoff" in result.stdout
    assert "missing tools/mirrorarc.py" not in result.stderr
...
def test_packaged_conversion_does_not_require_vault_wrapper_or_local_report(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "conversion"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    guide = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "conversion", "--guide"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    scaffold = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "conversion", "--init-results"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
...
def test_packaged_migration_does_not_require_vault_wrapper_or_local_report(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "migration"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert worksheet.returncode == 0, worksheet.stderr or worksheet.stdout
    assert "# MirrorArc Migration Review Worksheet" in worksheet.stdout
    ...
    assert "# MirrorArc Legacy Folder Migration Runbook" in runbook.stdout
...
def test_packaged_overlap_does_not_require_vault_wrapper_or_local_report(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "overlap", "--max-pairs", "1"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    worksheet = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "overlap", "--worksheet", "--max-pairs", "1"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
...
def test_packaged_mirrorarc_cli_runs_target_vault_commands(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    env = {**os.environ, "PYTHONPATH": str(ROOT / "src")}
    ...
    benchmark = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "benchmark"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    pilot = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "pilot"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert "_meta/source-manifest.json: missing; run `mirrorarc sync` first." in conversion.stdout
    ...
    assert "# MirrorArc Migration Review Worksheet" in migration_worksheet.stdout
    ...
    assert "# MirrorArc Legacy Folder Migration Runbook" in migration_runbook.stdout
    ...
    assert "# MirrorArc Frontmatter Domain Normalization Worksheet" in migration_normalize_worksheet.stdout
    ...
    assert "# MirrorArc Recovery Worksheet" in recovery_worksheet.stdout
    ...
    recovery_runbook = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "recovery", "--runbook"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert "# MirrorArc Recovery Runbook" in recovery_runbook.stdout
    ...
    assert "Run `mirrorarc sync` before handoff" in m365.stdout
    ...
    overlap_worksheet = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "overlap", "--worksheet"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert "# MirrorArc Overlap Calibration Worksheet" in overlap_worksheet.stdout
    assert "No note bodies, shared terms, source text, or reviewer notes are included." in overlap_worksheet.stdout

    catalog = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "catalog"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    catalog_check = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "catalog", "--check"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    catalog_html = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "catalog", "--html"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    catalog_html_check = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "catalog", "--html", "--check"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
...
def test_mirrorarc_overlap_report_calibrates_thresholds_without_content(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_overlap_notes(vault)

    result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "overlap"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    worksheet = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "overlap", "--worksheet"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    as_json = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "overlap", "--json", "--max-pairs", "1"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_benchmark_reports_result_scores_without_answer_content(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    ...
    assert (
        "mirrorarc_markdown: results=1 score=2/2 avg=2.00 corrections=0 violations=0 "
...
def test_mirrorarc_benchmark_accepts_legacy_document_chat_mode(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
...
def test_mirrorarc_benchmark_accepts_legacy_product_mode(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    task_path = vault / "_meta" / "agent-readiness-tasks.yml"
    ...
    result_pack = yaml.safe_load(result_path.read_text(encoding="utf-8"))
    for item in result_pack["results"]:
        if item["mode"] == "mirrorarc_markdown":
    ...
    assert "warning: comparison_modes uses legacy vaultwright_markdown; prefer mirrorarc_markdown" in result.stdout


def test_mirrorarc_benchmark_warns_on_uncited_scored_result(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    (vault / "_meta" / "agent-readiness-results.yml").write_text(
...
def test_mirrorarc_benchmark_require_citations_fails_on_uncited_scored_result(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    (vault / "_meta" / "agent-readiness-results.yml").write_text(
...
def test_mirrorarc_benchmark_require_prompt_safety_fails_on_missing_or_violation(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    (vault / "_meta" / "agent-readiness-results.yml").write_text(
...
def test_mirrorarc_benchmark_require_results_fails_on_incomplete_scores(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
...
def test_mirrorarc_benchmark_rejects_answer_text_and_reviewer_notes(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    (vault / "_meta" / "agent-readiness-results.yml").write_text(
...
def test_mirrorarc_benchmark_rejects_top_level_private_result_fields(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    (vault / "_meta" / "agent-readiness-results.yml").write_text(
...
def test_mirrorarc_benchmark_rejects_unrelated_citations(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
...
def test_mirrorarc_benchmark_rejects_non_finite_elapsed_seconds(tmp_path: Path) -> None:
...
def test_mirrorarc_benchmark_require_results_requires_task_pack(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)

    result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "benchmark", "--require-results"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_benchmark_default_result_file_requires_task_pack(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    (vault / "_meta" / "agent-readiness-results.yml").write_text(
        "schema_version: 1\ncorpus: fixture\nresults: []\n",
...
def test_mirrorarc_benchmark_init_tasks_writes_private_scaffold_from_manifest(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
...
def test_mirrorarc_benchmark_init_tasks_refuses_existing_pack_without_force(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_manifest_fixture(vault)
...
def test_mirrorarc_benchmark_init_results_writes_private_scaffold(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    result_path = vault / "_meta" / "agent-readiness-results.yml"
    ...
    assert {
        ...
        (f"{family}-1", mode)
        for family in ("answer", "reconcile", "update", "audit", "consolidate")
        for mode in ("raw_source_folder", "plain_markitdown_dump", "mirrorarc_markdown")
    }
...
def test_mirrorarc_benchmark_init_results_refuses_existing_pack_without_force(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
...
def test_mirrorarc_benchmark_worksheet_prints_private_run_sheet_without_paths(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    ...
    assert "#### mirrorarc_markdown" in result.stdout
...
def test_packaged_mirrorarc_cli_runs_benchmark_result_args_without_local_runtime(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    ...
    assert payload["result_summary"]["modes"]["mirrorarc_markdown"]["score"] == 2
    assert payload["result_summary"]["modes"]["mirrorarc_markdown"]["source_citations"] == 1
    assert payload["result_summary"]["modes"]["mirrorarc_markdown"]["generated_mirror_citations"] == 1
    assert payload["result_summary"]["modes"]["mirrorarc_markdown"]["prompt_safety_reviewed"] == 1
...
def test_packaged_mirrorarc_cli_runs_benchmark_prompt_safety_gate_without_local_runtime(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    (vault / "_meta" / "agent-readiness-results.yml").write_text(
    ...
    payload = json.loads(result.stdout)
    assert payload["result_summary"]["modes"]["mirrorarc_markdown"]["prompt_safety_reviewed"] == 1
    assert payload["result_summary"]["modes"]["mirrorarc_markdown"]["prompt_safety_violations"] == 0
    assert "missing tools/mirrorarc.py" not in result.stderr
    assert "benchmark_tasks.py" not in result.stderr


def test_packaged_mirrorarc_cli_runs_benchmark_init_results_without_local_runtime(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    remove_local_benchmark_runtime(vault)
...
def test_packaged_mirrorarc_cli_runs_benchmark_init_tasks_without_local_runtime(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_manifest_fixture(vault)
    remove_local_benchmark_runtime(vault)
...
def test_packaged_mirrorarc_cli_runs_benchmark_worksheet_without_local_runtime(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    write_agent_benchmark_fixture(vault)
    remove_local_benchmark_runtime(vault)
...
def test_packaged_mirrorarc_cli_init_from_source_checkout(tmp_path: Path) -> None:
    target = tmp_path / "new-vault"
    env = {**os.environ, "PYTHONPATH": str(ROOT / "src"), "MIRRORARC_REPO": str(ROOT)}

    result = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "init", str(target)],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert (target / "tools" / "mirrorarc.py").exists()
    assert f"mirrorarc --root {target} doctor" in result.stdout
    assert f"mirrorarc --root {target} status --json" in result.stdout
    assert "python3.11 tools/mirrorarc.py" not in result.stdout


def test_packaged_mirrorarc_cli_init_from_packaged_template(tmp_path: Path) -> None:
    target = tmp_path / "packaged-vault"
    env = {**os.environ, "PYTHONPATH": str(ROOT / "src")}
    env.pop("MIRRORARC_REPO", None)
    ...
    assert (target / "tools" / "mirrorarc.py").exists()
    assert f"mirrorarc --root {target} doctor" in result.stdout
    assert f"mirrorarc --root {target} sync --json" in result.stdout
    assert "python3.11 tools/mirrorarc.py" not in result.stdout
...
def test_mirrorarc_conversion_report_prioritizes_spot_checks(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    ...
    json_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "conversion", "--json"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    guide_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "conversion", "--guide"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    guide_json_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "conversion", "--guide", "--json"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_pilot_report_summarizes_evidence_without_content(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    sync = load_sync_module()
    ...
    json_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "pilot", "--json"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    worksheet_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "pilot", "--worksheet"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    ...
    assert benchmark_results["modes"]["mirrorarc_markdown"]["prompt_safety_reviewed"] == 1
    assert benchmark_results["modes"]["mirrorarc_markdown"]["prompt_safety_violations"] == 0
    ...
    assert "# MirrorArc Pilot Evidence Summary" in worksheet_result.stdout
...
def test_packaged_pilot_does_not_require_vault_wrapper_or_local_reports(tmp_path: Path) -> None:
    ...
    worksheet = subprocess.run(
        [sys.executable, "-m", "mirrorarc.cli", "--root", str(vault), "pilot", "--worksheet"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert worksheet.returncode == 0, worksheet.stderr or worksheet.stdout
    assert "# MirrorArc Pilot Evidence Summary" in worksheet.stdout
...
def test_mirrorarc_m365_report_summarizes_handoff_without_content(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    sync = load_sync_module()
    repo_fixture_id = sync.repo_id_for("local/fixture", "fixture.md")
    ...
    result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "m365"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    json_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "m365", "--json"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_catalog_and_m365_surface_unconfigured_repo_mirror_before_resync(tmp_path: Path) -> None:
    ...
    catalog_json = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "catalog", "--json"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    catalog_md = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "catalog", "--stdout"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    catalog_html = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "catalog", "--html", "--stdout"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_review_ledger_records_hashes_without_artifact_content(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    source = vault / "40_delivery" / "client-plan.docx"
    mirror = vault / "_mirrors" / "40_delivery" / "client-plan.md"
    ...
    summary = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "review"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    summary_json = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "review", "--json"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    ...
    check = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "review", "--check"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_sandbox_report_checks_copied_boundary_without_content(tmp_path: Path) -> None:
    source_root = tmp_path / "original-documents"
    source_root.mkdir()
...
def test_mirrorarc_sandbox_reports_profile_neutral_nested_git_boundary(tmp_path: Path) -> None:
    source_root = tmp_path / "original-documents"
    source_root.mkdir()
    parent = tmp_path / "parent"
    parent.mkdir()
...
def test_packaged_mirrorarc_sandbox_does_not_require_vault_wrapper_or_local_sandbox_runtime(
    tmp_path: Path,
) -> None:
    source_root = tmp_path / "original-documents"
    source_root.mkdir()
    ...
    assert "missing tools/mirrorarc.py" not in result.stdout
    assert "missing tools/mirrorarc.py" not in result.stderr
    ...
    assert payload["report"]["inventory"]["dedicated_generated_mirrors"] == 1
    assert "missing tools/mirrorarc.py" not in json_result.stdout
    assert "missing tools/mirrorarc.py" not in json_result.stderr
...
def test_mirrorarc_sandbox_report_fails_when_source_root_is_vault(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
...
def test_mirrorarc_conversion_report_handles_invalid_inputs(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)

    invalid_args = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "conversion", "--low-risk-per-format", "-1"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_conversion_quality_results_are_metadata_only(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    ...
    scaffold = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "conversion", "--init-results"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_mirrorarc_migration_reports_legacy_and_unknown_folders(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "migration"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    json_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "migration", "--json"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    worksheet_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "migration", "--worksheet"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    runbook_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "migration", "--runbook"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    normalize_preview = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "migration", "--normalize-frontmatter-domains"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    ...
    assert worksheet_result.returncode == 0, worksheet_result.stderr or worksheet_result.stdout
    assert "# MirrorArc Migration Review Worksheet" in worksheet_result.stdout
    ...
    assert "# MirrorArc Legacy Folder Migration Runbook" in runbook_result.stdout
    ...
    assert "Resolve `mirrorarc recovery --worksheet` items before trusting generated mirrors." in runbook_result.stdout
    ...
    assert "# MirrorArc Frontmatter Domain Normalization Worksheet" in normalize_worksheet.stdout
    ...
    assert "mirrorarc migration --normalize-frontmatter-domains --write" in normalize_worksheet.stdout
...
def test_mirrorarc_recovery_reports_manifest_actions(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    sync = load_sync_module()
        ...
        json.dumps(
            {
                ...
                "records": [
                    ...
                    },
                    {
                        ...
                        "lifecycle_state": "conflict",
                        "errors": [
                            "Source bytes match multiple missing manifest records; MirrorArc cannot choose the correct source history automatically."
                        ],
                    },
                ],
            }
    ...
    result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "recovery"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    ...
    worksheet_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "recovery", "--worksheet"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    ...
    assert worksheet_result.returncode == 0, worksheet_result.stderr or worksheet_result.stdout
    assert "# MirrorArc Recovery Worksheet" in worksheet_result.stdout
    ...
    assert "State explanation: MirrorArc retains the mirror and manifest record for review instead of deleting evidence." in worksheet_result.stdout
    ...
    runbook_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "recovery", "--runbook"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    ...
    assert runbook_result.returncode == 0, runbook_result.stderr or runbook_result.stdout
    assert "# MirrorArc Recovery Runbook" in runbook_result.stdout
    ...
    json_result = subprocess.run(
        [sys.executable, str(vault / "tools" / "mirrorarc.py"), "recovery", "--json"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    ...
    assert by_id["src-moved"]["lifecycle"]["explanation"] == (
        "MirrorArc found a likely move but will not strand or duplicate generated mirrors automatically."
    )
...
def test_mirrorarc_recovery_reports_refresh_and_planned_states(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    sync = load_sync_module()
...
def test_mirrorarc_recovery_reports_stale_atomic_temp_files(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    shutil.copytree(ROOT / "template", vault)
    mirror = vault / "_mirrors" / "40_delivery" / "registration.md"
...
def test_office_lifecycle_guidance_explains_review_states() -> None:
    ...
    assert any("source_missing (2): MirrorArc retains the mirror and manifest record for review instead of deleting evidence." in line for line in lines)
    assert any("Next: Locate or restore the source." in line for line in lines)
    assert any("source_moved (1): MirrorArc found a likely move but will not strand or duplicate generated mirrors automatically." in line for line in lines)
...
def test_repo_sync_marks_manifest_record_unconfigured_when_config_entry_removed(tmp_path: Path) -> None:
    ...
    assert runbook.returncode == 0, runbook.stderr or runbook.stdout
    assert "# MirrorArc Recovery Runbook" in runbook.stdout
