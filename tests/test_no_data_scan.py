...
def test_no_data_scan_default_skips_ignored_legacy_state(tmp_path: Path) -> None:
    ...
    state.parent.mkdir()
    state.write_bytes(b"SQLite format 3\0synthetic legacy local state")
...
def test_no_data_scan_blocks_staged_legacy_state(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    subprocess.run(["git", "init"], cwd=repo, check=True, capture_output=True)
    (repo / ".gitignore").write_text(".vaultwright/\n", encoding="utf-8")
...
def test_no_data_scan_does_not_luhn_scan_opaque_pdf_bytes(tmp_path: Path) -> None:
...
def test_no_data_scan_allows_provenance_listed_example_office_files() -> None:
    path = ROOT / "examples/ontario-grid-evidence-vault/70_outputs/briefings/ontario-grid-evidence-briefing.pptx"
