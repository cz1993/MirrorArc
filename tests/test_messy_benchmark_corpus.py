...
def test_generate_messy_benchmark_corpus_reviewed_results_validate_after_sync(tmp_path: Path) -> None:
    ...
    assert modes["noeticweave_markdown"]["score"] == 10
    assert modes["noeticweave_markdown"]["max_score"] == 10
    assert modes["noeticweave_markdown"]["reviewer_corrections"] == 0
    assert modes["noeticweave_markdown"]["generated_mirror_citations"] == 5
    assert "NW-MESSY" not in benchmark.stdout
...
def test_generate_messy_benchmark_corpus_rejects_source_checkout_target() -> None:
    ...
    assert "outside the NoeticWeave source checkout" in result.stderr
