...
def test_government_services_agent_readiness_tasks_reference_committed_sources() -> None:
    ...
    assert "noeticweave_markdown: results=6 score=12/12 avg=2.00 corrections=0" in public_results.stdout
...
def run_example_regeneration(tmp_path: Path, name: str, generated_rels: list[Path], raw_folder_rels: list[Path]) -> str:
    ...
    cli_plan = subprocess.run(
        [sys.executable, str(vault / "tools" / "noeticweave.py"), "plan"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
    ...
    second_sync = subprocess.run(
        [sys.executable, str(vault / "tools" / "noeticweave.py"), "sync"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def run_noeticweave(vault: Path, command: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, str(vault / "tools" / "noeticweave.py"), command],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_government_services_example_mirrors_regenerate_from_sources(tmp_path: Path) -> None:
    ...
    benchmark = subprocess.run(
        [sys.executable, str(vault / "tools" / "noeticweave.py"), "benchmark", "--require-generated"],
        cwd=vault,
        text=True,
        capture_output=True,
    )
...
def test_northwind_recovery_gate_regenerates_and_flags_review_states(tmp_path: Path) -> None:
    ...
    first_sync = run_noeticweave(vault, "sync")
    ...
    lint = run_noeticweave(vault, "lint")
    ...
    missing_status = run_noeticweave(vault, "status")
    ...
    manual_status = run_noeticweave(vault, "status")
