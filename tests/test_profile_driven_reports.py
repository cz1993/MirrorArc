...
def test_package_cli_catalog_separates_profile_machine_owned_markdown(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "noeticweave.cli", "--root", str(vault), "catalog", "--json"],
        cwd=ROOT,
        env=package_cli_env(),
        text=True,
        capture_output=True,
...
def test_package_cli_repo_sync_uses_profile_repo_notes_dir(tmp_path: Path) -> None:
    ...
    sync = subprocess.run(
        [sys.executable, "-m", "noeticweave.cli", "--root", str(vault), "sync"],
        cwd=ROOT,
        env=package_cli_env(),
        text=True,
        capture_output=True,
...
def test_package_cli_repo_sync_uses_profile_context_fields(tmp_path: Path) -> None:
    ...
    lint = subprocess.run(
        [sys.executable, "-m", "noeticweave.cli", "--root", str(vault), "lint"],
        cwd=ROOT,
        env=package_cli_env(),
        text=True,
        capture_output=True,
    ...
    annotations = subprocess.run(
        [sys.executable, "-m", "noeticweave.cli", "--root", str(vault), "migrate", "annotations", "--plan", "--json"],
        cwd=ROOT,
        env=package_cli_env(),
        text=True,
        capture_output=True,
...
def test_package_cli_overlap_uses_profile_inactive_status_flags(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "noeticweave.cli", "--root", str(vault), "overlap", "--json", "--max-pairs", "3"],
        cwd=ROOT,
        env=package_cli_env(),
        text=True,
        capture_output=True,
...
def test_package_cli_benchmark_reads_profile_task_pack(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "noeticweave.cli", "--root", str(vault), "benchmark", "--json"],
        cwd=ROOT,
        env=package_cli_env(),
        text=True,
        capture_output=True,
